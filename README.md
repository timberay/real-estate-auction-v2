...
| **F02. 물건 수집** | 두 가지 방식으로 경매 물건을 수집합니다. **사건번호 검색**: 사건번호 입력 시 60개 법원을 자동 순회하여 해당 물건을 찾고 상세 데이터를 수집합니다. **조건검색**: 관심 지역과 예산 범위에 맞는 물건을 일괄 검색하여 인라인으로 표시하고, 클릭 한 번으로 내 목록에 추가합니다. | 완료 |
| **F03. 물건분석** | 수집된 물건의 권리관계를 자동 분석합니다. AI(LLM) 기반 위험 검사 항목 자동 평가, 수동 체크리스트 병행, 최종등급 탭에서 권리분석 리포트와 입찰 판단을 종합합니다. 6개 LLM 제공자 지원(Anthropic, OpenAI, Gemini, OpenRouter, Ollama, Mock). | 진행중 |
