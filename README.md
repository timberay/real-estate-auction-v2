...
| **F02. 물건분석 (5탭+최종등급)** | PDF 업로드 → AI(LLM) 자동 분석(권리분석·물건분석 약 30~35개 항목) + 수동 입력. 89개 점검항목을 5개 탭(권리분석 27, 물건분석 13, 현장확인 13, 수익분석 29, 입찰&낙찰 7)으로 분류. 최종등급 탭에서 종합 안전등급 산출. | 완료 |

### 확장 기능 (P1)

| 기능 | 설명 | 진행사항 |
|---|---|---|
| **F03. 순수익 계산기** | 모든 세금과 비용을 차감한 실제 순수익을 계산합니다. 역산 모드(목표 수익 → 최대 입찰가). 세금 계산 복잡도가 높아 P1으로 이동. | 완료 |
| **F05. 분석 리포트 PDF 내보내기** | 물건 분석 결과를 오프라인 전문가 상담용 PDF로 내보냅니다. 전문가 상담 가이드(법무사/세무사/은행/대출 컨설턴트) 포함. | 완료 |

### 성장 기능 (P2)

| 기능 | 설명 | 진행사항 |
|---|---|---|
| **F06. 명도 시나리오 가이드** | 명도 프로세스 단계별 가이드(15단계 + 9개 분기), 33개 질문 기반 시뮬레이터로 난이도 예측, 법률 근거 제공. 실행은 오프라인 전문가 상담으로. | 완료 |
