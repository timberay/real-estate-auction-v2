...
class Inspection::PdfPromptBuilderTest < ActiveSupport::TestCase
  ...
  # --- B6 / E-12: amount_type field ---

  test "prompt includes amount_type field documentation" do
    prompt = Inspection::PdfPromptBuilder::SYSTEM_PROMPT
    assert_match(/amount_type/, prompt, "rights_timeline schema must include amount_type field")
  end

  test "prompt explains 채권최고액 inflation" do
    prompt = Inspection::PdfPromptBuilder::SYSTEM_PROMPT
    assert_match(/채권최고액/, prompt, "prompt must mention 채권최고액")
    assert(
      prompt.match?(/110/) || prompt.match?(/120/) || prompt.match?(/실제 채권액과 다를 수 있/),
      "prompt must explain that 채권최고액 differs from actual debt (110~120% or '실제 채권액과 다를 수 있')"
    )
  end

  test "prompt enumerates amount_type values" do
    prompt = Inspection::PdfPromptBuilder::SYSTEM_PROMPT
    enum_values = %w[채권최고액 원금 청구금액 확정채권액 기타]
    matched = enum_values.count { |v| prompt.include?(v) }
    assert matched >= 3, "prompt must mention at least 3 of #{enum_values.inspect}, got #{matched}"
  end
end
