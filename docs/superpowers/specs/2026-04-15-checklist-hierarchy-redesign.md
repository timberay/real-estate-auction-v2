...
```ruby
def skip_for?(answered_results_by_code, all_items_by_code: {})
  ...
  if parent_item&.skip_for?(answered_results_by_code, all_items_by_code: all_items_by_code)
    return true
  end
...
### `checklist_dependency_controller.js` — cascade hide
...
- Parent shown → only direct children shown (grandchildren depend on intermediate parent's answer)
