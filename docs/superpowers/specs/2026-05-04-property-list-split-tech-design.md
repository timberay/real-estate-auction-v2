...
resources :search_results, only: [ :index, :create ] do
  collection do
    delete :clear                    # 기존 유지 — 의미상 삭제 액션
  end
  member do
    post :import                     # 기존 유지
    post :inline_import              # 기존 유지 — /search_results/:id/inline_import (RESTful, params[:id])
  end
end
...
기존 라우트 구조(`delete :clear`, `member do post :inline_import`)를 그대로 유지하고 `index` 액션만 추가한다. `inline_import`는 단일 SearchResult를 처리하므로 `member` 라우트가 RESTful하며 `params[:id]`를 사용한다.
...
기존 `PropertiesController#index`의 검색 페이지네이션 로직(2026-04-20 추가된 100건 제한 + 20건 단위 페이지) 전체를 그대로 마이그레이션한다. 누락 시 대량 조회 API 비용 폭탄 regression 위험.
...
def index
  ...
  # 페이지네이션: 20건/페이지, 최대 100건 (외부 API 제한)
...
- `@existing_case_numbers`는 검색 결과 카드의 "이미 추가됨" 배지 판단용 (Q2=A 결정)
- **변경점**: 기존 `PropertiesController#index`는 `where.not(case_number: existing_case_numbers)`로 이미 추가된 항목을 **숨겼다**. 본 작업에서 검색 결과에 그대로 노출하고 view에서 배지로 분기하는 방식으로 전환한다 (Q2=A).
- `@total_pages`, `@search_page`, `@api_total_count`, `@over_api_limit`는 기존 페이지네이션/안내 UI에서 그대로 사용된다.
...
검색 결과 카드 클릭 시 호출. Turbo Stream 응답으로 페이지에 머문다. `params[:id]` 그대로 사용 (member route).

```ruby
def inline_import
  search_result = current_user.search_results.find(params[:id])
  perform_import(search_result)
...
**변경점**: 기존 `inline_import`는 `turbo_stream.append("property-cards-grid", ...)`로 "내 물건" 그리드에 새 카드를 덧붙이는 응답을 포함했다. 페이지 분리 후 `/search` 페이지에는 `property-cards-grid` 자체가 존재하지 않으므로 **append 스트림은 제거**한다. 카드를 "이미 추가됨" 상태로 `replace` 하는 스트림만 남긴다.

`dom_id(search_result, :inline)`은 Rails 표준 헬퍼로 기존 `_inline_result_item.html.erb`의 카드 요소 id와 일치해야 한다 (구현 시 기존 form id 컨벤션 확인).
...
렌더 위치: `app/components/header/component.html.erb` 의 `<div class="flex items-center gap-1">` (다크모드 토글 + 프로필 아바타를 감싸는 컨테이너) 내부, 다크모드 토글 **앞**에 partial 삽입. 글로벌 헤더는 `Header::Component`로 분리되어 있어 layout 직접 수정 없이 컴포넌트 ERB만 변경하면 된다.
...
- 카드 클릭 영역이 카드 전체인지 특정 버튼인지 — 현재 카드 자체를 form submit으로 감싸는 방식 가정 (기존 `_inline_result_item.html.erb` 의 form 구조 확인 필요)
- 검색 결과에 "이미 추가됨" 항목이 다수일 때 페이지네이션 UX — 기존에는 `where.not`으로 제외했으나 본 spec에서는 노출 + 배지 처리. 페이지당 사용 가능한 추가 카드 수가 줄어드는 trade-off 수용
