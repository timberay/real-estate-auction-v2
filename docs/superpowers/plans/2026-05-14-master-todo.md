...
| Theme | 핵심 질문 | 상태 |
|-------|----------|------|
| **1. 계산 엔진 신뢰성** | "숫자가 진짜 맞나?" — 베테랑 retention | T1.1 완료, T1.2 진행 중 |
| **2. 운영 가시성 + 안전망** | "깨지면 보이고, 사용자 작업이 보호되나?" | 대기 |
...
| **T1.2** | **양도세 매트릭스 정밀화** | **진행 중** | W1-3 / C23 / E-25 |
| T1.3 | 소액임차인 최우선변제 자동 계산 (DividendSimulator) | 대기 | W1-4 / C25 / E-27 |
| T1.4 | 회차별 저감률 + 낙찰가 통계 시드 | 대기 | W2-1 / C22 / E-24 |
| T1.5 | DSR 한도 계산 (BudgetSetting 확장) | 대기 | W2-2 / C26 / E-29 |

**T1.2 스코프 결정 (2026-05-14)**:
- C-4 패턴 미러: TransferTaxRate 모델 + seed + Calculator + Stimulus 매트릭스 주입
- 차원: tier × holding_period × regulated_region (BudgetSetting 기존 컬럼 재사용, 신규 UI 없음)
- 후속 분리: 양도세 admin UI, 1주택자 9억 누진세, 12억 비과세 정밀 모드
...
| T2.3 | 컨트롤러 bang-method rescue systemic fix (`Controllers::ErrorRescue` concern, 19개소 일괄) | A1 |
